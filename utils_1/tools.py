...
def generate_rwr_subgraph(dgl_graph, subgraph_size):
    ...
    # print(traces[0], len(traces))

    traces = dgl.sampling.random_walk(dgl_graph, all_idx, restart_prob=0.,
                                                            length=subgraph_size * 3 - 1)
    # print(traces[0][1], len(traces[0]))
    ...
    for i, trace in enumerate(traces[0]):
        ...
        while len(subv[i]) < reduced_size:
            ...
            cur_trace = dgl.sampling.random_walk(dgl_graph, [i], restart_prob=0.1,
                                                            length=subgraph_size * 5 - 1)
            # subv[i] = torch.unique(torch.cat(cur_trace[0]), sorted=False).tolist()
            subv[i] = torch.unique(cur_trace[0][0], sorted=False).tolist()
            retry_time += 1
