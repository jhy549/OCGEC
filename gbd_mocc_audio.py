...
if __name__ == "__main__":
    ...
    val_dataset = test_dataset[1*len(test_dataset) // 10:5 * len(test_dataset) // 10]
    train_dataset = dataset[2 * len(dataset) // 40: 8* len(dataset) // 40]
    print("train set: ", len(train_dataset))
    print("valid set: ", len(val_dataset))
    print("test set: ", len(test_dataset))

    datamodule = LightningDataset(train_dataset, val_dataset, test_dataset,
                                batch_size=1, drop_last=True, num_workers=1)#, num_workers=1
    # print(datamodule)

    #Load Model
    model = GINModule(in_channels=102, hidden_channels=128, out_channels=2,
                      )
    ...
    trainer = L.Trainer(precision=16,
                        ...
                        accelerator='gpu',
                        devices=devices,
                        max_epochs=3, log_every_n_steps=5,
