...
from models import ModelDataset_cifar_old
...
def main(args):
    ...
    dataset = ModelDataset_cifar_old(data_dir=data_dir)
    test_dataset = ModelDataset_cifar_old(data_dir=test_dir)
    # print(dataset.num_features)
    dataset = dataset.shuffle()
    test_dataset= test_dataset.shuffle()
    # test_dataset = test_dataset
    test = test_dataset[2 * len(test_dataset) // 10:4 * len(test_dataset) // 10]
    ...
    # deep_SVDD = TrainerDeepSVDD(train_test_config['device'],1,model=model,dataloader=train_loader,pooler=pooler)
...
if __name__ == '__main__':
    ...
    #python main_svdd_cifar.py  --device cuda:0 --max_epoch 20 --num_hidden 32 --pooling mean