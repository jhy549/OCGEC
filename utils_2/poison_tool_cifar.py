...
class DatasetBD(Dataset):
    def __init__(self, args, full_dataset, inject_portion, transform=None, mode="train", device=torch.device("cuda"), distance=1):
        self.dataset = self.addTrigger(full_dataset, 0, inject_portion, mode, distance, 3, 3, 'gridTrigger', 'all2one')
        self.device = device
        self.transform = transform
    ...
    def __getitem__(self, item):
        ...
        if self.inject_portion:
            return self.transform(img), label, self.dataset[item][2]
        else:
