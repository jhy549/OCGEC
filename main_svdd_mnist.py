...
from models import ModelDataset_mnist,ModelDataset
...
def main(args):
    ...
    data_dir = "/home/jianghaoyu/train_500"
    ...
    train_dataset = dataset[2 * len(dataset) // 10:4 * len(dataset) // 10]
    ...
    model.load_state_dict(torch.load("GAEmodel_mnist.pt"))
    print('model build finish')
    deep_SVDD = TrainerDeepSVDD(train_test_config['device'],3,model=model,dataloader=train_loader,pooler=pooler)
...
if __name__ == '__main__':
    ...
    #python main_svdd_mnist.py  --device cuda:2 --max_epoch 2 --num_hidden 32