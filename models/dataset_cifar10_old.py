...
class ModelDataset_cifar_old(Dataset):
    ...
    def load_model_struct_from_path(self, path):
        ...
        with torch.no_grad():
            ...
            # print(shadow_model.linear.weight.shape)
            pad = nn.ZeroPad2d(padding=(0, 0, 128, 128))
            linear_node = pad(linear_node)
            # print(linear_node.shape)
            nodes_feat.append(torch.reshape(linear_node, (1, 262656))[0])
            # get fc node
            fc_index = cnt
            cnt += 1
