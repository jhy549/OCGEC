...
def epoch_meta_train_oc(meta_model, basic_model, optimizer, dataset):
    ...
    for i in range(len(dataset)):
        ...
        out = basic_model(x,train_pos_edge_index)
...
class MetaClassifierOC(nn.Module):
    def __init__(self, input_size=64, class_num=64, N_in=50, gpu=True):
...
if __name__ == "__main__":
    ...
    model = GAE(GCNEncoder(262656, 64))
    #通过 load_state_dict 函数加载参数，torch.load() 函数中重要的一步是反序列化。
    model.load_state_dict(torch.load("GAEmodel.pt"))
